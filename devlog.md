...
- 学習ログは 1日 1ファイルで管理する
- ファイル名は YYYY-MM-DD.md 形式にする
- 一覧では新しい日付が上に来る
- 詳細な内容は該当日のファイルに記録する

## 日付別ログ

- [devlog/2026-09-03.md](devlog/2026-09-03.md)
- [devlog/2026-09-02.md](devlog/2026-09-02.md)
- [devlog/2026-09-01.md](devlog/2026-09-01.md)
- [devlog/2026-08-31.md](devlog/2026-08-31.md)
- [devlog/2026-08-30.md](devlog/2026-08-30.md)
- [devlog/2026-05-09.md](devlog/2026-05-09.md)
- [devlog/2026-05-08.md](devlog/2026-05-08.md)

> それ以前の履歴は、同じく [devlog](devlog) フォルダに日付ごとに保存してある。
...
`server/index.js` は、React から来た通信を受け取り、`data.json` を読んだり保存したりするための API サーバー。
...
React 側（例: `http://localhost:5173`）と Express 側（例: `http://localhost:3001`）はポートが違うため、ブラウザからは別サイト扱いになる。
そのため CORS 許可が必要。

### app.use(express.json()) の理解

```js
app.use(express.json());
```

React から送った JSON を Express 側で `req.body` として読めるようにする設定。
...
React から「データをください」と呼ばれたときに `data.json` を読んで返す API。
GET は「読む」通信。
...
React から送られた最新データを `data.json` に保存する API。
PUT は「更新・保存」通信。

### readData / writeData の理解
...
`readData` は、`data.json` を読み込んで JSON 文字列を JavaScript オブジェクトに戻す。
...
`writeData` は、JavaScript オブジェクトを JSON 文字列に変換して `data.json` に保存する。

- `JSON.parse()` は JSON 文字列 -> JavaScript データ
- `JSON.stringify()` は JavaScript データ -> JSON 文字列
...
この 3 行は `server/data.json` の場所を正確に作るためのコード。
Node.js に JSON ファイルの場所を教えるために必要。
...
`try...catch` を使うと、`data.json` が壊れていてもサーバー全体が落ちにくくなる。
...
保存前に `patients` と `records` が配列か確認する。
変な形のデータを保存すると、React 側の `patients.map()` などが壊れるため。

### 今日の理解

Express サーバーは、React と `data.json` の間にいる「受付係」の役割。

- React は直接 `data.json` を触らない
- Express に「読んで」「保存して」を依頼する

### 今後の考え方

`server/index.js` は最初から全部自力で書けなくてもよい。
実務でも既存コード・ドキュメント・テンプレートを見ながら作ることが多い。

今はこのコードを「ローカル JSON 保存 API のひな型」として使う。
...
## 2026-04-20 学習ログ（react-hook-form の handleSubmit と自分で作る関数の違いを整理）

### 今日やったこと

今日は主に、`handleSubmit` という名前が2種類あるように見えて混乱していたところを整理した。
...
- 上は自分で定義した関数なのか
- 下の handleSubmit は react-hook-form のものなのか
- 無名関数と名前つき関数の違いは何か
- 分けて書くべきか、その場で書いてもよいのか

を整理した。

### 1. react-hook-form の handleSubmit と自分の関数は別物

同じ handleSubmit という名前でも、中身は同じではない。

react-hook-form の handleSubmit

これは useForm() から取り出すライブラリの関数。
...
### 2. 下の <form onSubmit={handleSubmit(...) }> の読み方
...
つまり、
分けなくても動くが、理解しやすさのために分けることがある。

### 5. handleSubmit という名前を自分で使うと混乱しやすい

今日の学びとしてかなり大きかったのはここ。

react-hook-form にも handleSubmit があり、
自分でも `const handleSubmit = async (data) => { ... }` と書けてしまう。
...
のように、役割がわかる名前をつける方が整理しやすい と理解した。
...
が useForm() より前に書いてあっても、
その中の reset() はその場ですぐ実行されるわけではない。
...
なので、
関数の中で reset() を使っていても、送信時にはもう使える状態だから大丈夫
ということを理解した。

### 7. 今日のまとめ

今日は、handleSubmit という名前が出てきた時に、

- これは react-hook-form のものか
- これは自分で作った関数か
